...
namespace Poker
{
    public partial class frmPoker : Form
    {
        ...
        // === 下注功能 ===
        int balance = 1000000;      // 玩家餘額
        int betAmount = 0;       // 本次下注金額
        // ===============
        ...
        public frmPoker()
        {
            ...
            // === 下注功能 ===
            nudBet.Maximum = balance;
            lblBalance.Text = $"餘額：{balance}";
            // ===============
        ...
        private void btnCheck_Click(object sender, EventArgs e)
        {
            ...
            if (multiplier > 0)
                lblBetResult.Text = $"贏得 {winAmount} 元";
            else
                lblBetResult.Text = "未中獎";

            lblBalance.Text = $"餘額：{balance}";

            nudBet.Maximum = balance;
            if (nudBet.Value > balance) nudBet.Value = balance;
            // ===========================
        ...
        {
            betAmount = (int)nudBet.Value;
            if (betAmount > balance)
            {
                MessageBox.Show("餘額不足！", "下注失敗");
                return;
            }
            balance -= betAmount;
            lblBalance.Text = $"餘額：{balance}";
